...
import com.solid.example.service.TransactionServicePostgres;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		Transaction T1 = new Delivery(new TransactionServiceMySQL());
		T1.setClient(client);
		T1.setCounterpart(counterpart);
		T1.setValue(BigDecimal.valueOf(15000));
		T1.saveTransaction();
		
		Transaction T2 = new DeliveryToCompany(new TransactionServiceMySQL());
		T2.setClient(client);
		T2.setCounterpart(counterpart);
		T2.setValue(BigDecimal.valueOf(15000));
		T2.saveTransaction();
		
		Transaction P1 = new Payment(new TransactionServiceMySQL());
		P1.setClient(client);
